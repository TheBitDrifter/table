...
func TestEntryIndex_NewEntriesAndRecycle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ei := f.NewEntryIndex()
			schema := f.NewSchema()
			tbl, err := f.NewTable(schema, ei, table.FactoryNewElementType[int]())
			if err != nil {
				t.Fatal(err)
			}
			_, err = ei.NewEntries(tt.newEntriesCount, tbl)
			...
			for _, dID := range deleted {
				entries, err := ei.NewEntries(1, tbl)
...
func TestEntryIndex_UpdateIndex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ei.NewEntries(tt.entriesCount, tbl)
...
func TestEntryIndex_Reset(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			newEntries, err := ei.NewEntries(tt.expectedCount, tbl)
			testing_util.CheckError(t, ei.NewEntries, err)
...
func TestEntryIndex_Recyclable(t *testing.T) {
	...
	ei.NewEntries(totalEntries, tbl)
	...
	newEntries, err := ei.NewEntries(newEntriesCount, tbl)
